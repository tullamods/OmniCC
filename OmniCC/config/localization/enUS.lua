...
L.MinSizeDesc = "How large something must be to display cooldown text. 100 is the size of a normal action button, 80 is about the size of a pet action button, and 47 is about the size of a debuff on the Blizzard target frame"
...
L.RulePatternsDesc = "The names or parts of names of UI elements that this rule should apply to. Each pattern should be entered on a separate line."
L.RulePriority = "Priority"
L.RulePriorityDesc = "Rules are evaluated ascending order. The first match is the one that will be applied to a cooldown."
L.RuleRemove = REMOVE
L.RuleRemoveDesc = "Removes this rule"
L.Rules = "Rules"
L.RulesDesc = "Rules can be used to apply themes to specific elements of your UI. If no rules match for a particular UI element, then it will use the default theme."
L.Rulesets = "Rulesets"
L.RuleTheme = "Theme"
...
L.Themes = "Themes"
L.ThemesDesc = "A theme is a collection of OmniCC apperance settings. Themes can be used in conjunction with rules to change OmniCC's on specific parts of your UI"
